...
class _EditTourScreenState extends State<EditTourScreen> {
  ...
  void initState() {
    ...
    _destinationController = TextEditingController(text: tour.destination);
    _descriptionController = TextEditingController(text: tour.description);
    _rateController = TextEditingController(text: tour.rate.toStringAsFixed(1));
    _isActive = tour.isActive;

    // Khởi tạo danh sách ảnh từ tour hiện tại
    for (var imageUrl in tour.images) {
    ...
    // Khởi tạo danh sách lịch trình từ tour hiện tại
    for (var itinerary in tour.itinerary) {
      _itineraryControllers.add(TextEditingController(text: itinerary));
    }
  ...
  Future<void> _submit() async {
    ...
    if (imagesList.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Vui lòng nhập ít nhất 1 link ảnh!")),
      );
      return;
    }
    if (itineraryList.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Vui lòng nhập ít nhất 1 ngày lịch trình!")),
      );
      return;
    }
    ...
    double rate = double.tryParse(_rateController.text) ?? 5.0;

    String? error = await _firestoreService.updateTour(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Chỉnh sửa Tour", style: TextStyle(color: Colors.white)),
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 10),
              _buildTextField(_priceController, "Giá (VNĐ)", Icons.attach_money,
                  keyboardType: TextInputType.number),
              const SizedBox(height: 10),
              _buildTextField(_rateController, "Đánh giá (0.0 - 5.0)", Icons.star,
              ...
              Card(
                child: SwitchListTile(
                  ...
                  subtitle: Text(_isActive ? 'Tour đang hoạt động' : 'Tour tạm dừng'),
                  value: _isActive,
                  onChanged: (value) {
                    setState(() {
                      _isActive = value;
              ...
              const Text("Dán đường link ảnh vào bên dưới:",
              ...
              Align(
                ...
                child: TextButton.icon(
                  ...
                  label: const Text("Thêm ảnh khác",
                      style: TextStyle(
              ...
              _buildTextField(_descriptionController, "Mô tả chung", Icons.description,
                  maxLines: 3),

              const SizedBox(height: 25),

              // --- PHẦN 2: LỊCH TRÌNH (DYNAMIC LIST) ---
              _buildSectionTitle("Lịch trình (Itinerary)"),
              const Text("Nhập hoạt động chi tiết cho từng ngày:",
                  style: TextStyle(color: Colors.grey, fontSize: 13)),
              const SizedBox(height: 10),
              ...
              Align(
                ...
                child: TextButton.icon(
                  ...
                  icon: const Icon(Icons.add_circle, color: Palette.primary),
                  label: const Text("Thêm ngày tiếp theo",
                      style: TextStyle(
                          color: Palette.primary, fontWeight: FontWeight.bold)),
                ),
              ),
